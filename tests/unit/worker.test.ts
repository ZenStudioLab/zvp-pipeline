...
function createBossMock() {
  const registrations: Array<{ name: string; options: { localConcurrency: number }; handler: (jobs: Array<{ data: any }>) => Promise<void> }> = [];
  const createQueue = vi.fn(async () => undefined);
  const work = vi.fn(async (name: string, options: { localConcurrency: number }, handler: (jobs: Array<{ data: any }>) => Promise<void>) => {
...
describe('registerPipelineWorkers', () => {
  it('registers process and AI workers with configurable concurrency', async () => {
    const { boss, registrations, createQueue } = createBossMock();
    ...
    await registerPipelineWorkers({
      boss: boss as never,
      repository: {} as never,
      aiEnricher: { enrich: vi.fn(async () => ({ status: 'updated' })) } as never,
      processJob,
      concurrency: 7,
    });
    ...
    expect(registrations.map((registration) => registration.options.localConcurrency)).toEqual([7, 7]);
  });
  ...
  it('enqueues AI enrichment only for published non-dry-run results', async () => {
    ...
    const processRegistration = registrations.find((registration) => registration.name === PIPELINE_PROCESS_QUEUE);
