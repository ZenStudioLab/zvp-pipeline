...
const createAiEnricher = vi.fn(() => ({ enrich: vi.fn(async () => ({ status: 'updated' })) }));
...
describe('createPipelineWorker lifecycle', () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
    createPipelineRuntimeRepository.mockResolvedValue({ close: repositoryClose });
  });

  it('closes the repository when the worker stops', async () => {
    const { createPipelineWorker } = await import('../../src/worker.js');

    const worker = await createPipelineWorker({ databaseUrl: 'postgresql://example' });
    await worker.stop();
  ...
  it('stops pg-boss when repository bootstrap fails', async () => {
    createPipelineRuntimeRepository.mockRejectedValueOnce(new Error('bootstrap failed'));
    const { createPipelineWorker } = await import('../../src/worker.js');

    await expect(createPipelineWorker({ databaseUrl: 'postgresql://example' })).rejects.toThrow('bootstrap failed');
